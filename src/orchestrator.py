...
class HomelabOrchestrator:
    ...
    def setup_logging(self):
        ...
        log_file_path = os.path.join(os.path.expanduser("~"), "homelab_setup.log")
        logging.basicConfig(
            level=log_level,
...
╚═══════════════════════════════════════════════════════════════╝
    ...
    def run_system_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            scan_path = os.path.join(os.path.expanduser("~"), "network_scan.json")
        ...
        try:
            ...
            analysis_path = os.path.join(os.path.expanduser("~"), "system_analysis.json")
        ...
        try:
            ...
            audit_path = os.path.join(os.path.expanduser("~"), "security_audit.json")
    ...
    def run_questionnaire(self):
        ...
        if hasattr(sys, '_called_from_test') or not sys.stdin.isatty():
            ...
            subprocess.run([sys.executable, os.path.join(os.path.dirname(__file__), 'installer', 'questionnaire.py'), '--non-interactive'], check=True, timeout=300)
        else:
            questionnaire = Questionnaire()
            questionnaire.run()
        
        # Load the generated config
        user_config_path = os.path.join(os.path.expanduser("~"), "user_config.json")
    ...
    def generate_configurations(self):
        ...
        self.results['deployment_path'] = configurator.generate_all_configurations()
    ...
    def apply_security_hardening(self):
        ...
        for task in hardening_tasks:
            ...
            for cmd in task['commands']:
                try:
                    subprocess.run(cmd.split(), check=True, capture_output=True, timeout=60)
                except Exception as e:
...
"""
        ...
        update_path = os.path.join(scripts_dir, 'update.sh')
...
"""
        ...
        status_path = os.path.join(scripts_dir, 'status.sh')
