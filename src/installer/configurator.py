...
class Configurator:
    ...
    def __init__(self):
        self.user_config = self._load_json(os.path.join(os.path.expanduser("~"), "user_config.json"))
        self.network_scan = self._load_json(os.path.join(os.path.expanduser("~"), "network_scan.json"))
        self.system_analysis = self._load_json(os.path.join(os.path.expanduser("~"), "system_analysis.json"))
        self.security_audit = self._load_json(os.path.join(os.path.expanduser("~"), "security_audit.json"))
...
"""
    ...
    def _generate_searxng_service(self) -> str:
        """Generate SearXNG search engine service"""
        return """
  searxng:
...
"""
    ...
    def _generate_matrix_synapse_service(self) -> str:
        """Generate Matrix Synapse chat server service"""
        return """
  synapse:
...
"""
    ...
    def _generate_authelia_service(self) -> str:
        """Generate Authelia authentication service"""
        return """
  authelia:
...
"""
    ...
    def generate_secrets(self):
        ...
        self.secrets = {
            ...
            # New secrets for Matrix Synapse
            "matrix_registration_shared_secret": self.generate_password(64),
            "matrix_macaroon_secret_key": self.generate_password(64),
            # New secrets for Authelia
...
services:""")
        ...
        if self.user_config.get('enable_searxng', False):
            services.append(self._generate_searxng_service())

        if self.user_config.get('enable_matrix', False):
            services.append(self._generate_matrix_synapse_service())

        if self.user_config.get('enable_authelia', False):
            services.append(self._generate_authelia_service())
